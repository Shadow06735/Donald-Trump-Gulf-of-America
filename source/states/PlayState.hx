...
class PlayState extends MusicBeatState
{
	...
	public static var achievedGoodEnd:Bool = false;
	public static var cheatingDetected:Bool = false;
	...
	override public function create()
	{
		...
		startingSong = true;
		achievedGoodEnd = false;
		cheatingDetected = false;

		if (songName == 'america')
		{
			trail = new FlxTrail(dad, null, 4, 6, 0.3, 0.069);
			trail.alpha = 0;
			addBehindDad(trail);
		}
		...
		Application.current.window.title = "Friday Night Funkin': VS Donald J. Trump" + ' | ' + playin;
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		{
			scoreName = 'MAGA Points';
			missesName = 'Skill Issues';
			accuracyName = 'Americanness';
			ratingName = 'Class';
		}
		else
		{
		...
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', scoreName + ': {1} | ' + missesName + ': {2} | ' + accuracyName + ': {3} | ' + ratingName + ': {4}', [songScore, songMisses, str, finalRating]);
		else tempScore = Language.getPhrase('score_text_instakill', scoreName + ': {1} | ' + accuracyName + ': {2} | ' + ratingName + ': {3}', [songScore, str, finalRating]);
	...
	function openChartEditor()
	{
		...
		Application.current.window.title = "Friday Night Funkin': VS Donald J. Trump";
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (WeekData.getWeekFileName() == 'week1') {
						if (songName == 'america' && ratingPercent >= 0.75 && !cpuControlled) {
							achievedGoodEnd = true;
							if (ClientPrefs.data.streamerMode)
								FlxG.sound.playMusic(Paths.music('title'));
							else
								FlxG.sound.playMusic(Paths.music('ymca'));
						}
						else if (cpuControlled)
						{
							...
							FlxG.sound.playMusic(Paths.music('failed'));
	...
	override function beatHit()
	{
		...
		if (songName == 'america')
		{
			if (curBeat >= 32 && curBeat < 96 || curBeat >= 160 && curBeat < 288)
				trail.alpha = 0.3;
			else if (curBeat == 96 || curBeat == 288)
				trail.alpha = 0;
		}
		...
		for (name in achievesToCheck) {
			...
			else // any FC achievements, name should be "weekFileName_nomiss", e.g: "week3_nomiss";
			{
					...
					&& storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
				{
					...
					{
						if (WeekData.getWeekFileName() == 'week1') {
							if (campaignStars + starsGained == 10)
								unlock = true;
						}
						else {
